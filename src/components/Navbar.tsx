...
const Navbar = () => {
  ...
  return <header className={cn("fixed top-0 left-0 right-0 z-50 transition-all duration-300", isScrolled ? "bg-card/95 backdrop-blur-md shadow-md py-3" : "bg-transparent py-5")}>
      <div className="container mx-auto px-4 lg:px-8">
        ...
        {/* Mobile Menu */}
        <div className={cn("lg:hidden absolute top-full left-0 right-0 bg-card/98 backdrop-blur-lg shadow-lg transition-all duration-300 overflow-hidden", isMobileMenuOpen ? "max-h-96 py-4" : "max-h-0")}>
          <ul className="flex flex-col items-center gap-4 px-4">
            ...
            <Button variant="hero" size="sm" className="w-full max-w-xs mt-2" onClick={() => handleNavClick("#contact")}>
              Hire Me
            </Button>
          </ul>
        </div>
